...
const AI = {
  ...
  setApiKey(k) { localStorage.setItem(this.API_KEY, k.trim()); },
...
async function sendAIMessage(prompt) {
  ...
  if (!AI.getApiKey()) {
    const histEl = document.getElementById('ai-chat-history');
    if (histEl) {
      histEl.innerHTML += `<div class="ai-msg assistant">🔑 Önce yukarıdaki kutuya Anthropic API anahtarını gir.</div>`;
      histEl.scrollTop = histEl.scrollHeight;
    }
    // Scroll API key panel into view
  ...
  try {
    ...
    if (suggestions.length && area) {
      area.innerHTML = `<div class="upgrade-panel"><div class="upgrade-panel-title">✨ Önerilen Görevler</div>
        ${suggestions.map((s,i)=>`
          <div class="ai-suggestion" data-idx="${i}" style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border)">
            <div><strong>${escHtml(s.title)}</strong>${s.date?`<br/><small style="color:var(--text-muted)">${s.date}</small>`:''}</div>
    ...
    const msg = e.message === 'API_KEY_MISSING'
      ? '🔑 API anahtarı bulunamadı. Lütfen yukarıya gir.'
      : `❌ Hata: ${e.message}`;
    if (histEl) histEl.innerHTML += `<div class="ai-msg assistant">${msg}</div>`;
  }
}

function _updateApiKeyUI() {
  const key = AI.getApiKey();
  const status  = document.getElementById('ai-apikey-status');
  const clearBtn= document.getElementById('ai-apikey-clear');
  const input   = document.getElementById('ai-apikey-input');
  if (!status) return;
  if (key) {
    status.textContent  = '✅ API anahtarı kayıtlı — sohbet edebilirsin.';
    status.style.color  = 'var(--success)';
    if (clearBtn) clearBtn.style.display = '';
    if (input)    input.value = '';
  } else {
    status.textContent  = 'Henüz anahtar girilmedi.';
...
function initAIEvents() {
  ...
  document.getElementById('ai-apikey-clear')?.addEventListener('click', () => {
    localStorage.removeItem(AI.API_KEY);
    _updateApiKeyUI();
  ...
  const newViews = [
    { id:'ai',        icon:'✨', label:'AI Asistan',
      html:`<section class="view" id="view-ai">
        ...
        <!-- API Key Setup -->
        <div class="upgrade-panel ai-apikey-panel" id="ai-apikey-panel">
          <div class="upgrade-panel-title" style="margin-bottom:12px">🔑 Anthropic API Anahtarı</div>
          <p style="font-size:13px;color:var(--text-muted);margin-bottom:12px">
            AI asistanı kullanmak için kendi Anthropic API anahtarını gir.
            Anahtar yalnızca tarayıcında saklanır, hiçbir yere gönderilmez.<br>
            <a href="https://console.anthropic.com/keys" target="_blank" style="color:var(--accent)">console.anthropic.com/keys</a> adresinden ücretsiz alabilirsin.
          </p>
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <input type="password" id="ai-apikey-input" placeholder="sk-ant-..." style="flex:1;min-width:200px;border:1.5px solid var(--border);border-radius:8px;padding:10px 12px;font-size:14px;font-family:var(--font-body);color:var(--text);background:var(--bg);outline:none;" />
            <button type="button" class="btn-primary" id="ai-apikey-save">Kaydet</button>
            <button type="button" class="btn-ghost" id="ai-apikey-clear" style="display:none">Sil</button>
          </div>
          <div id="ai-apikey-status" style="font-size:12px;margin-top:8px;color:var(--text-muted)"></div>
        </div>

        <!-- Chat -->
        <div class="upgrade-panel ai-input-area" id="ai-chat-panel">
...
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => {
    try {
      ...
      console.error('❌ Upgrades hatası:', e);
    }
  }, 150);
});
