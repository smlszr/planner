...
function injectUpgrades() {
  ...
  views.forEach(v => {
    ...
    a.addEventListener('click', e => { e.preventDefault(); showView(v.id); });
    nav.appendChild(a);
  });

  // ── Override showView to handle new views ──────────────────
  const _origShowView = showView;
  window.showView = function(viewId) {
    _origShowView(viewId);
    // Extra renders for new views
...
function patchTaskModal() {
  // Wait for DOM then inject priority UI
...
document.addEventListener('DOMContentLoaded', () => {
  // Wait a tick for app.js to finish its own DOMContentLoaded
  setTimeout(() => {
    // Inject CSS
    const link = document.createElement('link');
    link.rel='stylesheet'; link.href='upgrades.css';
    ...
    console.log('✅ Upgrades loaded: Search · Priority · AI · Pomodoro · Habits · Analytics');
  }, 50);
});
