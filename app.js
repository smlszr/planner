...
// Set to true ONLY after you've filled in real Firebase credentials above
...
function toggleDarkMode() {
  ...
  saveState();
  showToast(state.darkMode ? '🌙 Dark mode on' : '☀️ Light mode on');
}
...
function renderDashboard() {
  ...
  document.getElementById('greeting-time').textContent = hour<12?'morning':hour<18?'afternoon':'evening';
...
function initEventListeners() {
  ...
  document.getElementById('auth-skip-btn').addEventListener('click',()=>{
    hideAuthGate();
    showToast('Using local storage — data stays on this device');
  });
...
function init() {
  ...
  // If Firebase not enabled, hide auth gate immediately
  if (!FIREBASE_ENABLED) {
    hideAuthGate();
  }
