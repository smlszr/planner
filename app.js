...
function openGoalModal(type, editId=null) {
  ...
  setVal('goal-title-input', '');
  setVal('goal-desc-input',  '');
  setVal('goal-start-input', defs.start);
  setVal('goal-end-input',   defs.end);
  setVal('goal-type-input',  type);
  setVal('goal-edit-id',     '');
  setTxt('goal-modal-title', editId ? `${GOAL_TYPE_TR[type]} Düzenle` : `Yeni ${GOAL_TYPE_TR[type]}`);
  ...
  if (editId) {
    ...
    if (g) {
      setVal('goal-title-input',  g.title);
      setVal('goal-desc-input',   g.description || '');
      setVal('goal-start-input',  g.startDate || defs.start);
      setVal('goal-end-input',    g.endDate   || defs.end);
      setVal('goal-edit-id',      editId);
...
function saveGoal() {
  ...
  const title     = getVal('goal-title-input').trim();
  const type      = getVal('goal-type-input');
  const editId    = getVal('goal-edit-id');
  const startDate = getVal('goal-start-input');
  const endDate   = getVal('goal-end-input');
  const parentId  = getVal('goal-parent-select') || null;
  if (!title) { document.getElementById('goal-title-input')?.focus(); return; }

  if (editId) {
    const g = state.goals[type].find(g => g.id === editId);
    if (g) Object.assign(g, { title, description: getVal('goal-desc-input'), startDate, endDate, parentId: parentId||null });
  } else {
    state.goals[type].push({ id:uid(), title, description:getVal('goal-desc-input'), completed:false, createdAt:Date.now(), startDate, endDate, parentId:parentId||null, type });
  }
...
function renderWeekPlanner() {
  ...
  // Show active goals for this week at the top
  const weekGoals = [];
  ['year','month','week'].forEach(type => {
    (state.goals[type]||[]).filter(g => {
      if(!g.startDate) return false;
      const gs = toDateStr(ws), ge = toDateStr(we);
      return g.startDate <= ge && (!g.endDate || g.endDate >= gs);
    }).forEach(g => weekGoals.push({...g, _goalType: type}));
  });

  if(weekGoals.length) {
    const goalBar = document.createElement('div');
    goalBar.style.cssText = 'grid-column:1/-1;display:flex;flex-wrap:wrap;gap:6px;padding:10px 0 4px;';
    weekGoals.forEach(g => {
      ...
      pill.addEventListener('click', () => openGoalDetail(g._goalType, g.id));
      goalBar.appendChild(pill);
    });
    board.appendChild(goalBar);
  }
  ...
  for (let i=0;i<7;i++) {
    ...
    addBtn.className='add-task-inline'; addBtn.textContent='+ Ekle';
